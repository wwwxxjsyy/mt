          ...
          <ul class="shoplist-up" style="margin-top:0.1rem;">
            <li v-for="(item,id) in data" :key="id" @click="handleDetail">
              ...
              <div class="center">
                ...
                <span class="center-one">
                  <i class="iconfont icon-star_full"></i>
                  {{item.wm_poi_score}}
                </span>
                月售
                <span class="saleNum">{{item.yueshou}}</span>
                <p>
                  <i>优惠可叠加</i>
                  <!-- 满减活动 -->
                  <span>{{discounts[id].info}}</span>
                </p>
              </div>
              <div class="right">
                <div class="right-price">
                  ￥<span>5</span><i class="iconfont icon-right-arrow"></i>￥<span class="priceP">8</span>
                </div>
                <a href="#">升级</a>
...
export default {
  ...
  methods: {
    ...
    handleDetail() {
      ...
      this.$router.push("/homegoods/goods");
    }
  },
  created() {
    this.$axios.get("/api/vip/?token=ef27099b73cf483b82a4ae2657b58c08").then(res => {
      var code = res.data.code;
      if (code === 200) {
        this.data = res.data.data.shop_data;
        this.discounts= res.data.data.discounts;
