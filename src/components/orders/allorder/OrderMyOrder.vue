<template>
                            <!-- 我的订单 -->
    <div class="ordermyorder">
        <!-- title -->
        <div class="ordermyorder-top">
            <h3>我的订单</h3>
        </div>
        <!-- menu -->
        <div class="ordermyorder-bottom">
            <div class="myordermenu">
                <a href="javascript:;" @click="one(checknum)">全部订单<span></span></a>
            ...
            <ul class="myorderdelite">
                <li v-for="(item,index) in allorder">
                    ...
                    <div class="orderdelite-center">
                        <div class="delite-shopnames">
                            <p class="delite-shopname">
                                <span v-for="(comm,index) in item.commodity">{{comm.commodityname}}</span>
                            </p>
                    ...
                    <div class="orderdelite-bottom">
                        ...
                        <router-link to="" v-show="!item.iscomment">评价</router-link>
                    </div>
                </li>
            </ul>
        </div>
...
export default {
    ...
    mounted(){
        for(var i in this.allorder){
            let allcommodity=this.allorder[i].commodity;
            let commoditycount=allcommodity.length;
            this.allcommoditylen.push(commoditycount);
        }
