<template>
                                <!-- 退款 -->
    <div class="orderremimburse">
        <!-- menu -->
        <div class="remimbursemenu">
            <a href="javascript:;" @click="one(checknum)">全部订单</a>
            <a href="javascript:;" @click="two(checknum)">待评价</a>
            <a href="javascript:;" @click="three(checknum)">退款<span></span></a>
        </div>
        <!-- 所有订单商品 -->
        <ul class="remimbursedelite">
            <li v-for="(item,index) in refunds">
                <!-- 单个订单商品详情 -->
                <div class="orderdelite-top">
                    <div class="orderdelite-top-shop">
                        ...
                        <a href="javascript:;" v-show="!item.isfulfill" class="delitestate-my" @click="$router.push({name:'OrderOrderState',params:{allorder:allorder,shopid:item.shopid}})">退款成功</a>
                    </div>
                </div>
                <!-- 单个订单商品价格 -->
                ...
                <div class="orderdelite-bottom">
                    ...
                    <router-link to="" class="tk">退款详情</router-link>
                </div>
            </li>
        </ul>
    </div>
...
export default {
    ...
    mounted(){
        for(var i in this.refunds){
            let allcommodity=this.refunds[i].commodity;
    ...
    methods:{
        postallorders(){
            Observer.$emit("allorder",this.allorder);
        },
        one(checknum){
            checknum=this.order
...
.remimbursemenu a:first-child{
    margin-left:10%;
}
.remimbursemenu a:last-child{
    margin-right:10%;
}
.remimbursemenu span{
    position:absolute;
